...
func getDotFilePath() string {
	...
	dotFile := usr.HomeDir + "/.go-local-git-stats-cli"
	return dotFile
}
...
// of each line and parses it to slice of strings.
...
// sliceContains return true if `slice` contains `value`
...
func dumpStringSliceToFile(repos []string, filePath string) {
	...
	ioutil.WriteFile(filePath, []byte(content), 0o755)
}
...
// scanGitFolders return a list of subfolders of `folder` ending with `.git`.
// Return the base folder of the repo, the .git folder parent.
